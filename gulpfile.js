...
{
	return gulp.src(source.html) // files to modidy
...
{
	return gulp.src(source.css) // files to modidy
		...
		.pipe(rename({ // rename with .min.css
			extname: ".min.css"
		}))
...
{
	return gulp.src(source.js) // files to modify
		.pipe(fileinclude()) // include files
		.pipe(babel()) // use babel
		.pipe(gulp.dest(dist.js))
		.pipe(uglify()) // uglify(compress)
		.pipe(rename({ // rename with .min.js
			extname: ".min.js"
		}))
...
{
	return gulp.src(source.img) // files to modify
...
{
	browsersync.init({ // init browser sync
		server: {
...
let build = gulp.series( // tasks for build
